...
func Load() *Config {
	return &Config{
		...
		Database: DatabaseConfig{
			...
			User:            getEnv("DB_USER", "aeroxebee"),
			Password:        getEnv("DB_PASSWORD", "aeroxebee"),
			DBName:          getEnv("DB_NAME", "aeroxebee"),
		...
		JWT: JWTConfig{
			...
			Issuer:          getEnv("JWT_ISSUER", "aeroxebee"),
		},
		Encryption: EncryptionConfig{
		...
		Queue: QueueConfig{
			...
			ConsumerGroupName:    getEnv("QUEUE_CONSUMER_GROUP", "aeroxebee-workers"),
		...
		Telemetry: TelemetryConfig{
			ServiceName:       getEnv("TELEMETRY_SERVICE_NAME", "aeroxebee-backend"),
