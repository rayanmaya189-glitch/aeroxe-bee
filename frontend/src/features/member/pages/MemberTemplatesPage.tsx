...
import { Plus, FileText, Sparkles, Loader2, Lightbulb } from 'lucide-react'
import { useToast } from '@/components/ui/Toast'

import { generateAITemplate } from '@/services/dashboard'
...
export function MemberTemplatesPage() {
  ...
  const { data: templates = [], isLoading } = useQuery({
    ...
    queryFn: async () => { const res = await api.get<ApiResponse<PaginatedResponse<Template>>>('/member/templates'); return res.data.data?.data || [] },
  })
        ...
        <div className="space-y-4">
          ...
          <div><label className="mb-1.5 block text-sm font-medium text-gray-300">Body</label>
            <div className="relative">
              <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} className="block w-full rounded-xl border border-white/[0.08] bg-white/[0.05] px-3.5 py-2.5 pr-10 text-sm text-gray-100 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 placeholder:text-gray-500" required />
