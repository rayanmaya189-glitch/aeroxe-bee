        ...
        >
          <motion.div variants={fadeInUp} className="mb-16 text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-lg text-gray-400">
              Four interconnected products — admin control, member self-service, mobile device nodes, and a high-performance backend.
            </p>
          </motion.div>
            ...
            {ECOSYSTEM.map((product) => (
              ...
              >
                ...
                <div className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-white/[0.06] bg-white/[0.03] px-3 py-1">
                  <span className="text-[10px] font-medium text-gray-500">{product.tech}</span>
                </div>
          ...
          <motion.div variants={fadeInUp} className="mt-12 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-8">
            <div className="text-center text-sm font-medium text-gray-400 mb-6">How it connects</div>
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center sm:gap-2">
              {['Android Devices', '→', 'MQTT Broker', '→', 'Go Backend', '→', 'Redis Queues', '→', 'Web Dashboard'].map((step, i) => (
